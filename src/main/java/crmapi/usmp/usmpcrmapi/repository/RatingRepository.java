...
public interface RatingRepository extends JpaRepository<Rating,Integer>{
    ...
    @Query("select concat(tr.rateit , ' stars') as rating, count(tr.rateit) as count from Rating tr where extract(month from tr.date) = extract(month from current_date) group by rateit order by rateit")
    List<Map<String, Object>> queryNumberMonth();

    @Query("select to_char(avg(tr.rateit), 'FM999999999.00') as average from Rating tr where extract(month from tr.date) = extract(month from current_date)")
